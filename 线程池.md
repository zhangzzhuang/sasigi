...
所谓线程池，就是将多个线程放在一个池子里面（所谓池化技术），在使用线程时，不是创建一个线程，而是从线程池中获取一个可用的线程，去执行我们的核心任务。线程池的关键在于它为我们管理了多个线程，不需要去关心线程池的创建或者毁销，只需要关注我们的核心任务，在需要执行任务时，从线程池中获取线程，任务执行完后线程不会被销毁，而是会重新放入线程池中，等待机会去执行任务。
...
假设一个服务器完成一项任务所需时间为：
    ...
    T1为创建线程的时间；T2为在线程中执行任务的时间；T3为销毁线程的时间。如果T1+T3远大于T2，则可以用线程池，以提高服务性能。
    

 - ### 一个线程池包括四部分

  * 线程池管理器：用于创建并且管理线程池，包括创建线程池，销毁线程池，添加新任务
  * 工作线程：线程池中线程，在没有任务时处于等待状态，可以循环的执行任务
  * 任务接口：每个任务必须实现的接口，以供工作线程调度任务的执行，它主要规定任务的入口，任务执行完后的收尾工作，任务的执行状态等
 ...
 - ### 使用线程池的好处
  * 减少在创建和销毁线程上所花的时间以及系统的开销
  * 如不适用线程池，有可能造成系统创建大量线程而导致消耗完系统内存以及“过度切换”
  * 提高相应速度


 - ### 工作流程
 线程池的任务就在于负责这些线程的创建，销毁和任务处理，参数传递,唤醒和等待
  * 创建若干线程，置入线程池中
  * 任务到达时，从线程池中取空闲线程
  * 取到空闲线程，立刻进行任务处理
  * 否则新建一个线程，并置入线程池中，执行3
  * 如果创建失败或者线程池满，根据策略选择返回错误或将任务置入处理队列，等待处理
  * 销毁线程池

 - ### 风险
  * 容易遭受并发风险，诸如死锁和同步错误，还容易遭受特定于线程池的少数其它风险，诸如与池有关的死锁、资源不足和线程泄漏。

 - ### 资源不足
  * 线程池的一个优点在于：相对于其它替代调度机制而言，它们通常执行得很好。但**只有恰当地调整了线程池大小**时才是这样的。 线程消耗包括内存和其它系统资源在内的大量资源。**除了Thread对象所需的内存之外，每个线程都需要两个可能很大的执行调用堆栈**。 除此以外JVM可能会为每个Java线程创建一个本机线程，这些本机线程将消耗额外的系统资源。最后**虽然线程之间切换的调度开销很小， 但如果有很多线程，环境切换也可能严重地影响程序的性能**。 如果线程池太大，那么被那些线程消耗的资源可能严重地影响系统性能。在线程之间进行切换将会浪费时间， 而且使用超出比您实际需要的线程可能会引起资源匮乏问题，因为池线程正在消耗一些资源，而这些资源可能会被其它任务更有效地利用。 除了线程自身所使用的资源以外，**服务请求时所做的工作可能需要其它资源，例如JDBC连接、套接字或文件。** 这些也都是有限资源，有太多的并发请求也可能引起失效，例如不能分配JDBC连接。

 - ### 线城池的使用
  java.uitl.concurrent.ThreadPoolExecutor类是线程池中最核心的一个类
```java 
    public class TheadPoolExector extends AbstractExecutorService{
...
```
- 上面构造器中各参数的含义:
 * corePoolSize：核心池的大小**，在创建线程池后，默认情况下，线程池中没有任何线程，而是等待有任务到来才创建线程去执行任务**，除非调用了prestartAllCoreThreads()或者prestartCoreThread()方法，从这2个方法的名字就可以看出，是预创建线程的意思，即在没有任务到来之前就创建corePoolSize个线程或者一个线程。默认情况下，在创建了线程池后，线程池中的线程数为0，当有任务来之后，就会创建一个线程去执行任务，当线程池中的线程数目达到corePoolSize后，就会把到达的任务放到缓存队列当中；
